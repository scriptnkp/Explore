...
// ── รายชื่อหน่วยงาน (ล็อกไว้ตายตัวจากคอลัมน์ A ของ PEA_Master ณ ตอนนี้ เพื่อให้หน้าแรกโหลดเร็ว ไม่ต้องเรียก API) ──
// ถ้าในชีทมีการเพิ่มหน่วยงานใหม่ในอนาคต ต้องมาเพิ่มชื่อในลิสต์นี้เอง (ไม่ไดนามิกแล้ว) ต้องสะกดตรงกับคอลัมน์ A เป๊ะๆ
const DEPARTMENTS = [
  'กฟจ.นครพนม (L)',
  'กฟส.ท่าอุเทน (XS)',
  'กฟส.ธาตุพนม (S)',
...
// ── Home: รายการหน่วยงาน (ค่าคงที่ ไม่เรียก API เพื่อให้โหลดไว) ──
function renderHome() {
  $crumb.textContent = '';
  const depts = DEPARTMENTS;
  if (!depts.length) { $app.innerHTML = '<div class="empty">ยังไม่มีข้อมูลหน่วยงาน</div>'; return; }
  $app.innerHTML = `
